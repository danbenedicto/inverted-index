...
typedef struct LLNodeStruct
{
	...
	int pending_delete;		// set to 1 if SLDestroy targets this node while iterators point here
	int reference_count;
} LLNode;
...
{
	LLNode *head;
	CompareFuncT compare_funct;
	DestructFuncT destruct_funct;
};
...
{
	SortedListPtr list;	// really just for access to destruct function
	LLNode *curr;	// just need a pointer to node in linked list
};
...
/*
 * DB: Not required. Destroys a generic linked list (not a SortedList).
 */
void LLDestroy(LLNode *head, DestructFuncT destruct_funct);

/*
 * Not required either. Either destroys a node in a linked list or marks
 * it for deletion.
 */
